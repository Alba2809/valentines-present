...
function MessageTimer({ message, time, handleFinish }) {
  ...
  return (
    <motion.div
      ...
      //   exit={{ y: 10, opacity: 0 }}
      className="absolute bottom-0 left-1/2 right-1/2 -translate-x-1/2 -translate-y-1/2 px-4 py-3 bg-white/50 backdrop-blur-sm dark:bg-gray-800/50 dark:backdrop-blur-sm rounded-lg w-fit flex items-center justify-center gap-x-2 shadow-xl"
    >
      {/* create a animation for the message, where each letter move up and down in different speeds each letter */}
      <p className="text-lg text-center text-gray-800 dark:text-gray-200 text-nowrap">
        {message}
      </p>
      ...
      <AnimatePresence mode="wait">
        <motion.p
          ...
          className="text-lg text-center text-gray-800 dark:text-gray-200"
